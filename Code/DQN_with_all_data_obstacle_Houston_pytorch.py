...
data = pd.read_csv("Houston-data-4.csv")
...
if __name__ == "__main__":
    ...
    for e in range(args.episodes):
        ...
        plt.legend(fontsize=15)
        plt.savefig(args.output_file_name + 'training_path_taken_episode_' + str(e))
        plt.pause(1)
        plt.close()
    ...
    with open(args.output_file_name+'stepcounts', 'w') as f:

        # using csv.writer method from CSV package
        write = csv.writer(f)

        write.writerow(stepcounts)

    print(f"Minimum steps = {min(stepcounts)} taken in the episode = {stepcounts.index(min(stepcounts)) + 1}")
